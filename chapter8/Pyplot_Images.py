...
for pos, (draw, style, column, frame) in enumerate(zip(
        (plt.contourf, plt.contour, plt.imshow),  # <callout id="co.pyplot.functions"/>
        (plt.cm.autumn, plt.cm.cool, plt.cm.spring),
        columns, frames)):
    # Select the subplot with 2 rows and 2 columns
    plt.subplot(2, 2, pos + 1)  # <callout id="co.pyplot.subplot"/>

    # Plot the frame
    draw(frame[frame.columns[:span]], cmap=style, aspect="auto")  # <callout id="co.pyplot.draw"/>

    # Add embellishments
    plt.colorbar()  # <callout id="co.pyplot.embellish"/>
    plt.title(column)
    plt.xlabel("Year")
